...
%
point of view, all these decisions can be incorporated in a Game Theory strategy. During our project, we have tried to find optimum strategies that can be applied to various cycling races. We have proposed a Game Theory model based on our research and an independent simulation used to verify our model. We continue the work of [ cite the papers here] and try to obtain a model that is as appripriate as real life cycling races. Using our simulation combined with the game theory model we believe that one should be able to replicate the status of any Professional cycling race. This is useful when comparing decisions taken by the racers during the race in order to improve their style (?).
